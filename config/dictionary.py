...
CREATE_PASSPHRASE_FOR_ADD_USER = u"""
Для добавления нового пользователя необходимо придумать пароль.
Этот пароль необходимо дать новому пользователю.
После того, как он напишет этот пароль мне в личку - пользователь будет добавлен.

Обрати внимание! До тех пор, пока новый пользователь не напишет мне пароль,
или пока ты не скажешь мне /cancel - бот не будет обрабатывать другие команды!"""
WAITING_FOR_NEW_USER = u"""
Ок, теперь дай этот пароль новому пользователю, пускай он напишет мне его в личку.
Если ты передумал - напиши мне /cancel"""
WAITING_FOR_NEW_USER_WAS_CANCELED = u"""Добавление нового пользователя остановлено"""
NEW_USER_WAS_ADDED = u"""Новый пользователь добавлен. Его id : {user_id}.
...
DELETE_USER_ID_MESSAGE = u"""Введите id, который нужно удалить."""
